...
#plot 2 row 1 col 2
plot(strptime(dfndf$datetime,"%d/%m/%Y %H:%M:%S"),dfndf$Voltage,type="l", xlab = "datetime", ylab = "Voltage")
...
legend("topright",legend=c("Sub_metering_1","Sub_metering_2","Sub_metering_3"),col=plot_colors,lwd=1, cex=1, xjust=.4, yjust=3, y.intersp=1,bty="n") 

#plot 4 row 2 col 2
plot(strptime(dfndf$datetime,"%d/%m/%Y %H:%M:%S"),dfndf$Global_reactive_power,type="l", xlab = "datetime", ylab = "Global Reactive Power")
